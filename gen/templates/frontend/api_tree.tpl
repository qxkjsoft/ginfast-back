...
{{- end}}
    children?: {{.StructName}}Data[]; // 子节点
}

export type {{.StructName}}TreeListResult = BaseResult<{
    list: {{.StructName}}Data[];
}>;

export type {{.StructName}}Result = BaseResult<{{.StructName}}Data>;

export const get{{.StructName}}TreeList = () => {
    return http.request<{{.StructName}}TreeListResult>("get", baseUrlApi("plugins/{{.DirName}}/{{.FileName}}/tree"));
};
