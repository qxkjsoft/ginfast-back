...
func main() {
	...
	outputFile := flag.String("output", "../../resource/database/sqlserver_converted.sql", "SQL Server SQL 输出文件路径（默认在输入文件名后加 _sqlserver.sql）")
	flag.Parse()
	...
	if *outputFile == "" {
		// 默认输出文件名：在输入文件名后加 _sqlserver.sql
		*outputFile = strings.TrimSuffix(*inputFile, ".sql") + "_sqlserver.sql"
	}
...
func (c *Converter) Convert() error {
	...
	// 写入 SQL Server 头部
	writer.WriteString("-- SQL Server SQL 转换文件\n")
	writer.WriteString("-- 由 MySQL SQL 转换而来\n\n")
	writer.WriteString("SET NOCOUNT ON;\n\n")
	...
	for _, line := range lines {
		...
		// 处理 DROP TABLE - SQL Server 语法
		if strings.HasPrefix(trimmedLine, "DROP TABLE IF EXISTS") {
			tableName = extractTableName(trimmedLine)
			wrappedTableName := wrapIdentifier(tableName)
			writer.WriteString(fmt.Sprintf("IF OBJECT_ID('%s', 'U') IS NOT NULL DROP TABLE %s;\n", tableName, wrappedTableName))
			continue
		}
...
func (c *Converter) processCreateTable(writer *bufio.Writer, tableName string, lines []string) {
	...
	// SQL Server 不支持表注释和列注释的 SQL 标准语法
	// 这里可以添加扩展属性设置，但为了简化，暂时不处理
	// 如果需要，可以添加 sp_addextendedproperty 语句
...
func (c *Converter) convertColumn(line string, tableInfo *TableInfo) string {
	...
	if strings.Contains(line, "varchar(") {
		...
		// 转换数据类型
		colType = "NVARCHAR(MAX)" // 默认类型
		// 首先尝试精确匹配类型（带括号）
		for mysqlType, sqlserverType := range typeMap {
			// 使用正则表达式匹配单词边界，避免部分匹配
			pattern := `\b` + regexp.QuoteMeta(mysqlType) + `\b`
			matched, _ := regexp.MatchString(pattern, line)
			if matched {
				colType = sqlserverType
				// 处理带占位符的类型，如 decimal(%d,%d)
				if strings.Contains(sqlserverType, "%d") && strings.Contains(mysqlType, "%d") {
					// 提取括号内的参数
					re := regexp.MustCompile(`(\w+)\((\d+),(\d+)\)`)
					matches := re.FindStringSubmatch(line)
					if len(matches) > 3 {
						colType = fmt.Sprintf(sqlserverType, matches[2], matches[3])
					}
				}
				break
			}
	...
	if isPrimary {
		...
		wrappedColName := wrapIdentifier(colName)
		return fmt.Sprintf("%s %s PRIMARY KEY", wrappedColName, colType)
	}
	...
	if strings.Contains(line, "DEFAULT") && !strings.Contains(line, "DEFAULT NULL") {
		...
		if len(matches) > 1 {
			...
			if strings.HasPrefix(defaultValue, "'") && strings.HasSuffix(defaultValue, "'") {
				// 检查是否为数值类型，如果是数值类型，去掉引号
				isNumericType := false
				// 检查列类型是否为数值类型
				numericTypes := []string{"INT", "BIGINT", "SMALLINT", "TINYINT", "DECIMAL", "NUMERIC", "FLOAT", "REAL", "BIT"}
				for _, numericType := range numericTypes {
					if strings.Contains(strings.ToUpper(colType), numericType) {
						isNumericType = true
						break
					}
				}
				...
				if isNumericType {
					// 去掉引号，只保留数字部分
					innerValue := defaultValue[1 : len(defaultValue)-1]
					// 检查是否为纯数字（可能包含小数点）
					if matched, _ := regexp.MatchString(`^\d+(\.\d+)?$`, innerValue); matched {
					...
					// 非数值类型：替换MySQL转义单引号 (\') 为SQL Server转义单引号 ('')
					defaultValue = strings.ReplaceAll(defaultValue, "\\'", "''")
				}
...
func (c *Converter) parseIndex(line string) *IndexInfo {
	...
	if len(matches) > 3 {
		...
		for i := range columns {
			...
			// 用方括号包裹列名
			columns[i] = wrapIdentifier(columns[i])
		}
	}
...
func (c *Converter) processInsert(writer *bufio.Writer, lines []string) {
	...
	// 提取表名
	var tableName string
	var wrappedTableName string
	reTable := regexp.MustCompile(`INSERT INTO\s+([^\s(]+)`)
	matches := reTable.FindStringSubmatch(insertSQL)
	if len(matches) > 1 {
		tableName = matches[1]
		wrappedTableName = wrapIdentifier(tableName)
		// 替换表名
		insertSQL = strings.Replace(insertSQL, tableName, wrappedTableName, 1)
	}
	...
	// 如果有 IDENTITY 列，需要添加列列表
	if hasIdentityColumn {
		// 匹配 INSERT INTO table VALUES (values)
		reInsert := regexp.MustCompile(`INSERT INTO\s+([^\s]+)\s+VALUES\s*\((.*)\)`)
		matches := reInsert.FindStringSubmatch(insertSQL)
		if len(matches) == 3 {
			table := matches[1]
			values := matches[2]
			// 获取列列表
			...
			if tableInfo, exists := c.tables[tableName]; exists {
				for _, col := range tableInfo.Columns {
					columns = append(columns, wrapIdentifier(col.Name))
				}
			}
			columnList := strings.Join(columns, ", ")
			// 重新构建 INSERT 语句
			insertSQL = fmt.Sprintf("INSERT INTO %s (%s) VALUES (%s)", table, columnList, values)
		}
	}

	// 如果有 IDENTITY 列，添加 IDENTITY_INSERT 设置
	if hasIdentityColumn && wrappedTableName != "" {
		writer.WriteString(fmt.Sprintf("SET IDENTITY_INSERT %s ON;\n", wrappedTableName))
	}
	...
	// 如果有 IDENTITY 列，关闭 IDENTITY_INSERT
	if hasIdentityColumn && wrappedTableName != "" {
		writer.WriteString(fmt.Sprintf("SET IDENTITY_INSERT %s OFF;\n", wrappedTableName))
	}
}
		...
		for _, index := range tableInfo.Indexes {
			if index.IsUnique {
				writer.WriteString(fmt.Sprintf("CREATE UNIQUE INDEX %s ON %s (%s);\n",
					index.Name, wrappedTableName, strings.Join(index.Columns, ", ")))
			} else {
...
// 包裹SQL Server标识符（表名、列名）用方括号
func wrapIdentifier(name string) string {
	// 如果已经包含方括号，直接返回
	if strings.HasPrefix(name, "[") && strings.HasSuffix(name, "]") {
		return name
	}
	// 如果包含点号（如 schema.table），需要分别包裹
