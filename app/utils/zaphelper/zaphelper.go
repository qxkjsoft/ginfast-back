...
func CreateZapFactory(entry func(zapcore.Entry) error) *zap.Logger {
	// 获取程序所处的模式：  开发调试 、 生产
	appDebug := g.ConfigYml.GetBool("AppDebug")
	...
	timePrecision := g.ConfigYml.GetString("Logs.TimePrecision")
	...
	switch g.ConfigYml.GetString("Logs.TextFormat") {
	case "console":
	...
	//写入器
	fileName := g.BasePath + g.ConfigYml.GetString("Logs.ZapLogName")
	lumberJackLogger := &lumberjack.Logger{
		Filename:   fileName,                              //日志文件的位置
		MaxSize:    g.ConfigYml.GetInt("Logs.MaxSize"),    //在进行切割之前，日志文件的最大大小（以MB为单位）
		MaxBackups: g.ConfigYml.GetInt("Logs.MaxBackups"), //保留旧文件的最大个数
		MaxAge:     g.ConfigYml.GetInt("Logs.MaxAge"),     //保留旧文件的最大天数
		Compress:   g.ConfigYml.GetBool("Logs.Compress"),  //是否压缩/归档旧文件
	}
